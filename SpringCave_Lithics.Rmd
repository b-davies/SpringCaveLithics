...
```{r}
#get unique cortical fragements used for lens clock analysis
...
print(sc_cortex_ratios)

```

#Step 5: Difference from "complete" assemblage using experimental data (Fig 3)
```{r}
  ...
  for (n in select_criteria){ #For each selection criteria
    for (i in frag_nums){ #For each number of fragments
      ...
      for (j in c(1:10000)){
        ...
        theo<-theo_nod(sy)
        nod<-nod_char_s(data,hrq_cobbles)
...
diffs<-sim/real
ww<-tibble(aug,criteria,perc,fr,diffs,log(cr))
sc2ktrueval<-log(sc_cortex_ratios[1])
sc2kcrs<-ww$`log(cr)`
sc2kp<-(length(sc2kcrs[sc2kcrs<=sc2ktrueval]) + length(sc2kcrs[sc2kcrs>=(sc2ktrueval*-1)]))/length(sc2kcrs)
...
diffs<-sim/real
ww<-tibble(aug,criteria,perc,fr,diffs,log(cr))
sctoptrueval<-log(sc_cortex_ratios[2])
sctopcrs<-ww$`log(cr)`
...
hist(sc2kcrs,xlim=c(-1,1),xlab="Log Cortex Ratio",main="Spring Cave >3k")
abline(v=sc2ktrueval,lty=2)
abline(v=sc2ktrueval*-1,lty=2)
...
#Step 6: Layer difference assessment Spring Cave (Fig 4)
```{r}

truediff0<-abs(sc_cortex_ratios[1]-sc_cortex_ratios[2])
diffs0<-c()
sc_all<-rbind(sc_pre3000,sc_post1000)
reduce2k<-length(sc_pre3000[,1])
total<-length(sc_all[,1])
for (i in c(1:10000)){
  ind<-sample(c(1:total),total-reduce2k,replace=FALSE)
  assemblage1<-sc_all[ind,]
  assemblage2<-sc_all[-ind,]
...
pv0<-length(diffs0[diffs0>=truediff0])/length(diffs0) + length(diffs0[diffs0<=(truediff0*-1)])/length(diffs0)
...
text(0.7,2000,paste("p=<0.001",sep=""))
